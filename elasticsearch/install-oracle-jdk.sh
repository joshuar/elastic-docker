...
# Ripped out of the great fedy Fedora post-installer:
# https://github.com/satya164/fedy

dlurl=$(curl -s -o - "http://www.oracle.com/technetwork/java/javase/downloads/index.html" \
    | tr ' ' '\n' | grep "/technetwork/java/javase/downloads/jdk8" | head -n 1 \
    | cut -d\" -f 2 | sed -e 's/^/http:\/\/www.oracle.com/')
rpmurl=$(curl -s -o - ${dlurl} | grep "Linux x64" | grep ".rpm" | cut -d\" -f 12 | grep -v demos | head -n 1)
rpmfile=${rpmurl##*/}

curl -s -L -o /tmp/${rpmfile} --header 'Cookie: oraclelicense=a' "${rpmurl}"

yum -y -q install /tmp/${rpmfile}
